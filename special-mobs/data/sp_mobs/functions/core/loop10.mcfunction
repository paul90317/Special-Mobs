...
execute if score #world spmobs.spnt matches 1 as @e[type=#sp_mobs:enimy,tag=!sp_mob.skill,tag=!sp_mob.ban] if score @s spm.rage_value >= #world spm.threshold at @s unless entity @a[distance=..16] run function sp_mobs:entity/wait_spawn/check_spawn

execute if score #world spmobs.ed matches 1 if predicate sp_mobs:1/4 run scoreboard players add @e[type=ender_dragon] spm.rage_value 10
