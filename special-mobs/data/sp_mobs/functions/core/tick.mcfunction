...
execute if score #world spm.intv.113 matches 50 if score #world spm.cf.spnt matches 1 as @e[type=#sp_mobs:enimy,tag=!sp_mob.skill,tag=!sp_mob.ban] if score @s spm.rage_value >= #world spm.threshold at @s unless entity @a[distance=..16] run function sp_mobs:entity/wait_spawn/check_spawn
